...
// mapStyle already defined above without extra brackets
// Removed stray lines
...
export function App() {
  ...
  async function submitGuess() {
    ...
    try {
      if (!guessCoords) return;
      if (!adminToken) {
        setNotice('⚠️ No session token – try reloading the page');
        return;
      }
  ...
  useEffect(() => {
    async function initSession() {
      try {
        const sessionRes = await fetch('/api/cet/session', {
          method: 'POST',
        });
        ...
        console.error(e);
        setNotice('⚠️ Could not start a session');
      }
    }
    initSession();
  }, []);
  ...
  useEffect(() => {
    fetch("/api/cet/all")
      .then((res) => res.json())
      .then((data) => {
        // The backend returns objects with {id, lat, lng, flag, image}
        // Convert them to the Sighting shape expected by the UI
        const loaded = data.map((row: any) => ({
          id: row.id,
          title: row.id,               // simple title; customize if needed
          description: "",
          latitude: row.lat,
          longitude: row.lng,
          status: "confirmed",         // default status; adjust later
          confidence: 100,
